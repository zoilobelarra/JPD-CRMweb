              ...
              <table>
                ...
                <tbody>
                  {loading ? (
                    ...
                    filtered.map(r => (
                      <tr key={r.id}>
                        <td className="fecha-cell">{fmtDate(r.fecha)}</td>
                        <td>
                          {r.accion?.valor
                        ...
                        <td className="description-cell" title={r.descripcion}>{r.descripcion || '—'}</td>
                        <td className="link-cell">
                          {r.hipervinculo
                            ? <a href={r.hipervinculo} target="_blank" rel="noreferrer">🔗 Abrir</a>
                            : <span style={{ color: 'var(--text3)' }}>—</span>}
                        </td>
                        <td className="description-cell" title={r.comentarios}>{r.comentarios || '—'}</td>
                        <td>
                          <div className="actions-cell">
                            <button className="btn btn-ghost btn-icon btn-sm" title="Editar" onClick={() => openEdit(r)}>✏️</button>
                            <button className="btn btn-danger btn-icon btn-sm" title="Eliminar" onClick={() => setConfirmDelete(r.id)}>🗑</button>
                          </div>
                        </td>
                      </tr>
                    ))
